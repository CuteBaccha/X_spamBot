...
from XspamBot import (OWNER_ID, HNDLR, SUDO_USERS, hl, LOGS_CHANNEL)
from pyrogram import Client, filters
from pyrogram.types import Message
from SpamX.data import *

Usage = f"**❌ Wrong Usage ❌** \n Type: `{HNDLR}help owner`"
Media = "XspamBot/downloads/Profile.jpg"
...
async def setpic(XspamBot: Client, e: Message):
...
async def setbio(xspam: Client, e: Message):
      ...
      if len(Rizoel) == 1:
          xd = str(Rizoel[0])
          ok = await XspamBot.get_me()
          nam = ok.first_name
          nam2 = ok.last_name
          try:
             await XspamBot.update_profile(first_name=nam, last_name=nam2, bio=xd)
